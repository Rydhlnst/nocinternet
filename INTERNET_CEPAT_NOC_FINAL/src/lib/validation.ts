...
export const schemas: Record<Exclude<Resource, "sites">, z.ZodObject<Record<string, z.ZodTypeAny>>> = {
  cids: z.object({ ...base, cid_number: z.string().trim().min(1, "Nomor CID wajib diisi").max(100), service_type: optionalText(100), bandwidth: optionalText(100), activation_date: optionalDate, pic: optionalText(150) }),
  ...
  upgrades: z.object({ ...base, current_bandwidth: z.string().trim().min(1, "Bandwidth saat ini wajib diisi").max(100), requested_bandwidth: z.string().trim().min(1, "Bandwidth permintaan wajib diisi").max(100), request_date: optionalDate, target_date: optionalDate, completion_date: optionalDate, pic: optionalText(150) }),
  maintenance: z.object({ ...base, maintenance_type: z.string().trim().min(1, "Jenis maintenance wajib diisi").max(120), scheduled_at: optionalDate, started_at: optionalDate, completed_at: optionalDate, impact: optionalText(300), pic_vendor: optionalText(150) }),
}
