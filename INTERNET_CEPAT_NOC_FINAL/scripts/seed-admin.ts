...
async function main() {
  ...
  const email = "info@internetcepat.co.id"
  const legacyEmail = "nocinternetcepat@gmail.com"
  const password = process.env.NOC_ADMIN_PASSWORD
  if (!password) throw new Error("NOC_ADMIN_PASSWORD is required")

  const db = getDb()
  const [targetUser] = await db.select().from(authUser).where(eq(authUser.email, email)).limit(1)
  const [legacyUser] = targetUser ? [] : await db.select().from(authUser).where(eq(authUser.email, legacyEmail)).limit(1)
  let userId = targetUser?.id ?? legacyUser?.id
  ...
  if (!userId) {
    ...
    if (legacyUser) await db.update(authUser).set({ email, updatedAt: new Date() }).where(eq(authUser.id, userId))
